...
APP_HOME=/data1/gateway/gateway-1.3.3.RELEASE.jar

#java虚拟机启动参数
JAVA_OPTS=
"-server  -Xms3g -Xmx3g -Xmn1g -Xss256k -XX:PermSize=256M
-XX:MaxPermSize=512M -XX:SurvivorRatio=8 -XX:MaxTenuringThreshold=5
...
-Xloggc:/data1/gateway/applogs/gc.log";
...
stop() {
   ...
   if [ $psid -ne 0 ]; then
      ...
      echo -e
      curl -H "Content-Type:text/plain;charset=UTF-8" -d ''  "http://localhost:8081/shutdown"
      sleep 10
      echo -e
